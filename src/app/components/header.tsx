"use client";
import {
  Sheet,
  SheetContent,
  SheetDescription,
  SheetHeader,
  SheetTitle,
  SheetTrigger,
} from "@/components/ui/sheet";

import Search from "./search";
import { useState } from "react";
import Link from "next/link";
import Image from "next/image";

export default function Header() {
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);

  const toggleSidebar = () => {
    setIsSidebarOpen(!isSidebarOpen);
  };

  return (
    <header>
      {/* First Layer */}
      <div className="bg-black text-white flex flex-col sm:flex-row items-center sm:justify-between md:justify-around p-3 sm:p-0">
        <div className="flex items-center mb-2 sm:mb-0">
          
          <p className="text-white text-sm flex text-[13px] justify-center p-3 font-extralight">
            Free Shipping On All Orders Over $50
          </p>
        </div>
        <div className="flex justify-end items-center font-extralight">
          <div className="flex gap-x-3 items-center">
            <div className="flex items-center">
              <label className="text-[13px]" htmlFor="dropdown">
                Eng
              </label>
              <Image width={200} height={200}
                className="w-4 h-4 flex" alt="arrow"
                src="data:image/png;base64,"
              />
            </div>
            <p className="text-[13px]">Faqs</p>
            <div className="flex items-center gap-x-1">
              
              <p className="text-[13px]">Need Help?</p>
            </div>
          </div>
        </div>
      </div>

      {/* Second Layer */}
      <div className=" flex items-center p-5 justify-between">
        <div className="flex gap-x-2 items-center justify-around">
          <Image width={50} height={50} src="/logo.PNG" alt="logo" />
          <p className="text-[26px] font-medium">Happy Feet</p>
        </div>
        <div className="flex items-center gap-x-5 ">
          <div className="">
            <Link href="/cart">
              <div className="md:w-[120px] md:h-[44px] p-3 ml-3 md:ml-0 bg-[#ffb132] hover:bg-[#876021] rounded-md flex items-center gap-x-2 justify-center text-[12px]">
                <Image width={200} height={200} className="w-[22px] h-[22px] " src="/cartwhite.png" alt="" />
                <p className="font-medium hidden md:block text-white">Cart</p>
                <p className="w-[8px] h-[8px] rounded-full mb-2 md:-ml-1 -ml-3 text-[10px] bg-[#ffffff] text-center  flex items-center justify-center text-white">
                  2
                </p>
              </div>
            </Link>
          </div>
          <div className="md:hidden">
          <Sheet>
            <SheetTrigger>
              <Image width={200} height={200} className="w-6 h-6" src="/menu.png" alt="" />
            </SheetTrigger>
            <SheetContent>
              <SheetHeader>
                <SheetTitle>
                  <div className="flex gap-x-2 items-center justify-center">
                    <Image width={200} height={200} src="/logo.PNG" alt="logo" />
                    <p className="text-[26px] font-medium">Happy Feet</p>
                  </div>
                </SheetTitle>
                <SheetDescription>
                  <hr />
                  <div className="flex flex-col text-[16px] gap-y-10 mt-10 text-left font-light ">
                    <Link href="/">
                      <p className="hover:text-[#ffb132] hover:font-semibold cursor-pointer">
                        Home
                      </p>
                    </Link>
                    <Link href="/contact" passHref>
                      <p className="hover:text-[#ffb132] hover:font-semibold">
                        Contact
                      </p>
                    </Link>
                    <Link href="/products" passHref>
                      <p className="hover:text-[#ffb132] hover:font-semibold">
                        Product
                      </p>
                    </Link>

                    <Link href="/about">
                      <p className="hover:text-[#ffb132] hover:font-semibold cursor-pointer">
                        About
                      </p>
                    </Link>
                  </div>
                </SheetDescription>
              </SheetHeader>
            </SheetContent>
          </Sheet>
          </div>
        </div>
      </div>

      {/* Third Layer (Navigation) */}
      <div className="hidden sm:flex justify-around items-center h-[74px]">
        <div className="flex justify-between gap-x-10 font-medium items-center text-[14px]">
          <Link href="/">
            <p className="hover:text-[#ffb132] hover:font-semibold cursor-pointer hidden lg:block">
              Home
            </p>
          </Link>
          <Link href="/contact" passHref>
            <p className="hover:text-[#ffb132] hover:font-semibold hidden lg:block">
              Contact
            </p>
          </Link>
          <Link href="/products" passHref>
            <p className="hover:text-[#ffb132] hover:font-semibold hidden lg:block">
              Product
            </p>
          </Link>
          <Link href="/about">
            <p className="hover:text-[#ffb132] hover:font-semibold cursor-pointer hidden lg:block">
              About
            </p>
          </Link>
        </div>

        {/* Search Bar */}
        <Search />
      </div>
    </header>
  );
}
